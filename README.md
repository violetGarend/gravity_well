...
| 🗺️ **执行之井** | `执行之井` `当前阶段` `进度` | 阶段性导航地图——展示当前阶段、里程碑进度、剩余时间窗口，不分配每日任务 |
...
我陷入了完美主义             → 5分钟快速干预
召开季度委员会               → 多视角复盘 + 规则修订
执行之井 / 当前阶段 / 进度     → 阶段性导航地图（当前阶段、里程碑、剩余时间）
加入待办[内容]               → 分析待办项，给出推荐归位（聚焦/冻结/待办池）
退出引力井                   → 恢复正常模式
```
...
│   ├── 执行.md                      ← 子系统5 阶段性导航算法 + 加入待办分析
│   ├── 执行看板模板.html            ← 执行之井可视化看板模板
│   └── 人格skills/                  ← 5 个人格视角文件
